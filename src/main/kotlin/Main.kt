...
fun prettyPrintNodes(
    ...
    nodes.forEach { node ->
        ...
        val nodeValue =
            if (node is Node.Primitive) {
                " value [${node.value.toUByteArray().prettyHex()}]"
            } else {
                ""
            }
...
fun parseTag(stream: ByteArrayInputStream): Tag {
    ...
    if (stream.available() <
        ...
        throw IllegalArgumentException("Unexpected end of stream, no length value found following tag ${tagDef.tag.prettyTagNameHex()}")
    }
    ...
    if (stream.available() <
        ...
        throw IllegalArgumentException("Unexpected end of stream, expected ${length - stream.available()} more byte(s)")
    }
    stream.read(value)
    ...
    if (tag.template() != null && !tag.template()?.tag.contentEquals(parentTag?.tag())) {
        if (parentTag == null) {
            throw IllegalArgumentException(
                "Tag ${tag.definition.tag.prettyTagNameHex()} expected to be nested under ${tag.template()?.tag?.prettyTagNameHex()}, but was at the root level",
            )
        }
        throw IllegalArgumentException(
            "Tag ${tag.definition.tag.prettyTagNameHex()} expected to be nested under ${tag.template()?.tag?.prettyTagNameHex()}, but was nested under ${parentTag.tag()?.prettyTagNameHex()}",
        )
    }

    if (tag.isConstructed()) {
        val childrenNodes =
            parseTags(ByteArrayInputStream(tag.value)).map { childTag ->
...
fun getSampleTlv(): ByteArray = Base64.getDecoder().decode("bxqEDjFQQVkuU1lTLkRERjAxpQiIAQJfLQJlbg==")

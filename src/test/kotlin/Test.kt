...
class Test {
    ...
    fun multipleConsecutiveTagsParse() {
        val bytes = byteArrayOf(0x6F.toByte(), 0x04.toByte(), 0x84.toByte(), 0x02.toByte(), 0x01.toByte(), 0x02.toByte(), 0x6F.toByte(), 0x04.toByte(), 0x84.toByte(), 0x02.toByte(), 0x03.toByte(), 0x04.toByte())
        val nodes = parseTags(ByteArrayInputStream(bytes)).map { tag -> buildNode(tag) }.toList()
    ...
    fun twoByteTagParses() {
        ...
        val bytes = byteArrayOf(0x6F.toByte(), 0x06.toByte(), 0xA5.toByte(), 0x04.toByte(), 0x5F.toByte(), 0x2D.toByte(), 0x01.toByte(), 0x11.toByte())
        val nodes = parseTags(ByteArrayInputStream(bytes)).map { tag -> buildNode(tag) }.toList()
        assertEquals(1, nodes.size)
        val node = (nodes.first() as Node.Constructed).children.first().let { it as Node.Constructed }.children.first()
        assertIs<Primitive>(node)
        with(node) {
    ...
    fun exceptionThrownIfNestedTagIsNotNested() {
        ...
        assertContains(exception.message!!, "Tag ShortFileIdentifier (88) expected to be nested under FCIProprietaryTemplate (A5), but was at the root level")
    }
    ...
    fun exceptionThrownIfTagIsNotNestedUnderProperTag() {
        ...
        assertContains(exception.message!!, "Tag ShortFileIdentifier (88) expected to be nested under FCIProprietaryTemplate (A5), but was nested under FCITemplate (6F)")
    }
